...
type ResearchScrollRestorationProps = {
  pagePath: string;
};

const SCROLL_KEY_PREFIX = 'sarec:research-scroll-y:';
const RESTORE_PATH_KEY = 'sarec:research-restore-path';
...
function saveResearchScrollPosition(pagePath: string) {
...
function shouldTrackHref(href: string, pagePath: string) {
  ...
  return url.origin === window.location.origin && normalizedCurrentPath === normalizedPagePath && normalizePath(url.pathname) !== normalizedPagePath;
}

function restoreResearchScrollPosition(pagePath: string) {
...
export function ResearchScrollRestoration({ pagePath }: ResearchScrollRestorationProps) {
  const normalizedPath = normalizePath(pagePath);
  const scrollKey = getScrollKey(normalizedPath);
  ...
  useLayoutEffect(() => {
    ...
    restoreResearchScrollPosition(normalizedPath);
  ...
  useEffect(() => {
    function handleClick(event: MouseEvent) {
      ...
      if (shouldTrackHref(anchor.href, normalizedPath)) {
        saveResearchScrollPosition(normalizedPath);
      }
    }
